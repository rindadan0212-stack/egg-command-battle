...
public class PicFrameSizeTests
{
    ...
    public void 骨組みのpic直書きノードの不一致を数える()
    {
        ...
        // ⭐ いま残っている6件は、すごろくの盤（`square`/`trail`）に古くからあるもの。
        //    絵（Kenney の 32ドット＝128px）が枠より大きい。
        //    ⚠️ **見た目は壊れていない**（`.n.card` が十分広く、絵は札の中に収まる）
        //    ── 嘘をついているのは数のほうで、そのぶん編集画面の「枠を絵に合わせる」と
        //    重なり検査が誤った数で動く。⚠️ 枠を正直な128pxにすると盤の見た目が変わるので、
        //    作者の判断を待って据え置いてある。
        // ⭐ **1件でも増えたら・直したら、ここが落ちる。**直したらこの表から消すこと。
        var 既知 = new[]
        {
            "square/plain", "square/mob",
            "trail/goalpic", "trail/rollpic", "trail/payarr", "trail/skippic",
        };
        var いま = mismatches
