...
public class RngGoldenTests
{
    ...
    /// tools/goldens.mjs の並びと同じ順で消費すること。</summary>
    [Fact]
...
public class SpeciesGoldenTests
{
    ...
    private static readonly Dictionary<string, string> Redrawn = new Dictionary<string, string>
    {
        ...
        // ⭐ 2026-08-25・段取り3。作者が描いた残り3枚（`art/handmade/sprite/*.png`）を取り込んだ。
        //    ⚠️ tamaru と同時に置き換わったので、色数はここで実測した値（取り込み時の SpriteImport
        //    の出力）に更新済み。null 対応前の色は PaletteResolveTests.ExpectedColors 側で押さえる。
        { "tsunoga", "2026-08-25 作者が描いた 64×64・18色へ差し替え" },
        { "haneru", "2026-08-25 作者が描いた 64×64・26色へ差し替え" },
        { "nobiru", "2026-08-25 作者が描いた 64×64・25色へ差し替え" },
        // ⭐ 2026-08-30・作者の指示「キャラクター画像は時間がかかるので64×64の仮絵」。
        //    tools/prepare-user-art.mjs が Pixelizer を通した7体。完成絵へ替えるまでの意匠。
        { "hirabe", "2026-08-30 64×64の仮絵へ差し替え" },
        { "togeru", "2026-08-30 64×64の仮絵へ差し替え" },
        { "marumi", "2026-08-30 64×64の仮絵へ差し替え" },
        { "kibane", "2026-08-30 64×64の仮絵へ差し替え" },
        { "iwao", "2026-08-30 64×64の仮絵へ差し替え" },
        { "homura", "2026-08-30 64×64の仮絵へ差し替え" },
        { "nushi", "2026-08-30 64×64の仮絵へ差し替え" },
    };
