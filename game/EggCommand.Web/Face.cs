...
public sealed class Face
{
    ...
    public string Text(string what, int row) => what switch
    {
        ...
        // 🔴 **`sub`（`c174　2代　変異0`）は消した**（2026-08-30・作者の指示「削除」）。
        //    ⚠️ `panel.txt`／`panelmini.txt` の節点も同時に外してある ── 片方だけ残すと
        //    誰も読まない枝になる。⭐ 家系図の「2代」は別の口（`Sheets.TreeSub`）なので無事。
    ...
    public string? Tint(string what) => what switch
    {
        ...
        // 🔴 **印と名前の側にも同じ色を掛ける**（2026-08-30・作者の指示「得意を緑、
        //    不得意を赤で表示」）。⚠️ 前は数（`wild`）だけが色付きで、▲▼ の印と
        //    ステの名前は灰のままだった ── 印は**どちらへ傾いているか**を言う欄なのに、
        //    色という同じ物差しがそこだけ効いていなかった。
        "key" or "wild" => RowInk(),
        _ => null,
    };
    ...
    /// <summary>⭐ **絵の下に敷く影の色**（2026-08-30・作者の指示「イラストの下に影を描写。
    /// キャラクターが映っている画面ではすべて同様に」）。
    ///
    /// 🔴 **出所はここ1つ。**⚠️ 画面ごとに濃さを書くと、同じ「影」が場所によって
    /// 違う濃さになり、同じ物だと読めなくなる（この作品で色が割れた事故と同じ型）。
    /// ⭐ 骨組みが持つのは**形と場所**（楕円の大きさ・絵への食い込み）だけ ──
    /// 絵の大きさが画面ごとに違うので、そこは骨組みでしか決められない。
    ///
    /// ⚠️ 新しい色ではない ── 地に置く薄墨で、`ink=faint` と同じ役
    /// （ただし透かす必要があるので `rgba`）。</summary>
    public const string ShadowCss = "rgba(20,24,40,.28)";
