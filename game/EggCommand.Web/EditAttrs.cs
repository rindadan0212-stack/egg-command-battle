...
namespace EggCommand.Web
{
    ...
    public static class EditAttrs
    {
        ...
        public static readonly IReadOnlyDictionary<string, string> Excluded = new Dictionary<string, string>
        {
            ...
            ["roll"] = "遊びの意味が変わる（背景を流す速さの名前）── 絵は「元＋左右反転」を"
                + "横に並べた2枚幅で作ってあり、値を変えると `stage.css` の級と対応が切れて"
                + "動きが止まる。⭐ 速さの出所は `stage.css` ひとつ（2026-08-27）",
            ["grow"] = "遊びの意味が変わる（巻物の中の `host` を切らない）── 触ると"
                + "盤が見えなくなる／押せなくなる方向に壊れうるので、飾りの表には載せない"
                + "（2026-08-26・すごろくの盤で実際に起きた不具合の直し）",
