...
function checkSp(sp,i){
  ...
  // ⭐ **大きさは 16 と 64 の両方が来る**（作者の手描きは 64×64）。⚠️ 16 を焼き付けない
  //    ── 塗る側はもともと両方に対応していたのに、ここだけ 16 で止めていた（2026-08-25 修正）。
  {
    const n=sp.sprite.length;
    if((n!==16&&n!==64)||sp.sprite.some(r=>r.length!==n))
      stop.push(`姿が ${sp.sprite[0]?sp.sprite[0].length:0}×${n}`
        +' ── 16×16 か 64×64 の正方形だけです（テキストで直してください）');
  }
  // ⚠️ 姿の添字が色数を超えていないか（遊びで描いた瞬間に落ちる）
  // 🔴 **番号は D.dotDigits の並びで数える。**⚠️ '1'─'9' だけ数えていたので、
  //    10 番以降（a─z）が黙って 0 になり、色数の検査が素通りしていた（2026-08-25 修正）。
  const big=Math.max(0,...sp.sprite.join('').split('')
    .map(c=>c==='.'?0:D.dotDigits.indexOf(c)+1));
  if(sp.palettes.some(p=>big>p.length))
    stop.push(`姿が色 ${big} 番を使っているのに、色が ${Math.min(...sp.palettes.map(p=>p.length))} つしかない`);
  // 🔴 **4色固定はもう規則ではない**（2026-08-21 に撤廃）。⚠️ ここだけ残っていたので、
  //    11 色のタマルを開くと保存できなかった（2026-08-25 修正）。
  //    ⭐ いまの規則は「1件の中で数を揃える」（Sheet.cs と同じ）。
  if(sp.palettes.length&&sp.palettes.some(p=>p.length!==sp.palettes[0].length))
    stop.push(`色の数が組ごとに違う（${sp.palettes.map(p=>p.length).join('/')}）`
      +' ── 1件の中では揃えてください（変異させた瞬間に落ちます）');
  if(sp.palettes.some(p=>p.length>D.dotDigits.length))
    stop.push(`色が ${Math.max(...sp.palettes.map(p=>p.length))} つ（上限 ${D.dotDigits.length}）`);
