...
namespace EggCommand.Sim
{
    ...
    public static class Sheet
    {
        ...
        public static string SkillHead()
        {
            ...
            md.Append("//   ⚠️ 帳面は通すが、実装した扱いにはならない（検査が ✍️ として数える）。\n");
            md.Append("//\n");
            md.Append($"// ⭐ CT に上限は無い。⚠️ ただし {Skills.CtCap} を超えると帳面は ⚠️ で通す\n");
            md.Append($"//   （盤面をひっくり返す技は {Skills.CtHeavy} まで黙る）。\n");
            // 🔴 **帳面が止めないことと、貼って通ることは別。**⚠️ ここを「止めはしない」で
            //    終えていたので、そのまま貼って `Skills.Audit` が落ちる形を招いていた（2026-08-25 修正）。
            md.Append("//   🔴 **ただし、そのまま貼ると `Skills.Audit` が落ちます。**帳面が止めないだけです。\n");
            md.Append("// 🔴 **確率が付くのは「相手に掛けるもの」だけ**（弱化・毒・スタン・挑発 など）。\n");
            md.Append("//   ⚠️ ダメージ・強化・リジェネ・シールドに付けると ⛔ になり、貼ると Audit が落ちます。\n\n");
            return md.ToString();
        }
        ...
        public static string SpeciesHead()
        {
            ...
            // ⚠️ **数と名前を本文に書かない。**⭐ 実データから起こす ── 手描きへ差し替えるたびに
            //    ここがずれる（2026-08-25 に「10種族／タマル」のまま実際は 7 と 4 になっていた）。
            var smallOnes = new List<string>();
            var bigOnes = new List<string>();
            foreach (var one in SpeciesTable.All)
                (one.Sprite.Width <= 16 ? smallOnes : bigOnes).Add(one.Name);
            md.Append($"//    ⭐ 16×16 の{smallOnes.Count}種族は 1=輪郭 2=体 3=差し色 4=目 で描いてある（仮置き）。\n");
            if (bigOnes.Count > 0)
                md.Append($"//    ⚠️ 64×64 の{bigOnes.Count}種族（{string.Join("・", bigOnes)}）は" + "この決めごとに従いません（色ごとに役が違います）。\n");
